...
class CompanyNames:
    ...
    def call_gemini(self, resume):
        ...
        Return 100 unique company names that hire candidates similar to this resume using Greenhouse.io or Lever.io. 
        The company names need to be simple plaintext and not contain any special characters e.g.
    ...
    def get_titles(self, parsed_response):
        job_titles = []
        for job in parsed_response["recommended_companies"]:
            job_titles.append(job['name'])
        return job_titles
    ...
    def save_to_csv(self, job_titles: list[str], filename: str = "company_names.csv"):
        ...
        filename = f"{DATA_PATH}company_names_{today}.csv"

        """Saves a list of job titles to a CSV file."""
        try:
            with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow(["Company Name"])  # Header row
    ...
    def full_run(self):
        ...
        names = self.get_titles(parsed_response)
        # rint(titles)
